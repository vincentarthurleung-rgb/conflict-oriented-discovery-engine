...
def run_l35_pmc_oa_stage(run_dir:str|Path, *, enabled:bool, network_enabled:bool=False, api_enabled:bool=False, max_papers:int=20, include_near_conflicts:bool=False, extractor:Callable|None=None, l1_client=None, l1_provider:str|None=None, l1_model:str|None=None, max_sections_per_paper:int=12, max_chunks_per_paper:int=24, max_chars_per_chunk:int=6000, max_total_chunks:int=200, l1_read_timeout_seconds:float=240, l1_max_retries:int=1, id_transport=None, oa_transport=None, download_transport=None)->dict:
    ...
    discovery_count=sum(1 for x in (artifacts/"fulltext_discovery_escalation_candidates.jsonl").read_text(encoding="utf-8").splitlines() if x.strip()) if (artifacts/"fulltext_discovery_escalation_candidates.jsonl").is_file() else 0
    handoff={"fulltext_escalation_candidate_count":discovery_count,"l35_candidate_paper_count":len(selection["candidate_papers"]),
             "fulltext_handoff_consistent":not enabled or not discovery_count or discovery_count==len(selection["candidate_papers"]),
             "fulltext_handoff_warnings":[] if not enabled or not discovery_count or discovery_count==len(selection["candidate_papers"]) else ["discovery_escalation_l35_count_mismatch"]}
    if not enabled or not selection["candidate_papers"]:
        status="not_enabled" if not enabled else "completed_no_candidates"; summary={"status":status,"candidate_paper_count":0,"pmcid_resolved_count":0,"oa_available_count":0,"fulltext_downloaded_count":0,"fulltext_l1_claim_count":0,"fulltext_confirmed_conflict_count":0,"copyright_safe":True,"non_oa_skipped_count":0,"message":selection["message"],**handoff}
