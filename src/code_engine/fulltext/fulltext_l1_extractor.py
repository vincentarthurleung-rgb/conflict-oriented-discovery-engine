        ...
        for paper in candidates:
            ...
            for section in sections:
                for chunk_index,text in enumerate(chunk_text(section["text"],max_chars_per_chunk)):
                    ...
                    planned.append((paper,section,text,digest,cid));selected.append({"chunk_id":cid,"chunk_hash":digest,"pmid":paper.get("pmid"),"pmcid":paper.get("pmcid"),"title":paper.get("title"),"section_title":section.get("section_title"),"section_type":section.get("section_type"),"section_index":section.get("section_index"),"chunk_index":chunk_index,"text":text,"source_scope":"full_text","selection_score":paper.get("selection_score",0.0),"selection_reasons":paper.get("selection_reasons",[]),"linked_observation_ids":paper.get("abstract_observation_ids",[]),"linked_weak_candidate_ids":paper.get("conflict_candidate_ids",[])})
