...
def replay(case_profile,search_plan,source_run,from_stage,output_root,output_suffix,bundle_id_suffix,*,no_l1=True,network=False,skip_fulltext=True,skip_l7=True,overwrite_bundle=False,bundle_root="case_bundles"):
    ...
    if from_stage=="l2":
        ...
        run_l2_abstract_step(run_dir=target,l1_mode="abstract_screening",execute=True,network=False,api=False)
        rerun.append("l2")
    ...
    if skip_l7:(artifacts/"l7_external_validation_summary.json").write_text(json.dumps({"status":"skipped","executed_validators":[],"skipped_validators":["all"],"reason":"stage_replay_skip_l7","network_used":False},indent=2)+"\n")
    if skip_fulltext:
        for name in ("l35_fulltext_retrieval_summary.json","l35_fulltext_l1_summary.json","l35_fulltext_conflict_confirmation_summary.json"):(artifacts/name).write_text(json.dumps({"status":"skipped","reason":"stage_replay_skip_fulltext","network_used":False,"candidate_paper_count":0},indent=2)+"\n")
    audit_result=l2_audit(target);write_l2_audit(audit_result,artifacts)
    manifest={"schema_version":"case_stage_replay_v1","source_run":str(source),"new_run":str(target.resolve()),"case_id":profile.case_id,"from_stage":from_stage,"reused_artifacts":reused,"rerun_stages":rerun,"skipped_stages":["acquisition","l1"]+(["fulltext"] if skip_fulltext else [])+(["l7"] if skip_l7 else []),"network_used":False,"llm_used":False,"created_at":datetime.now(timezone.utc).isoformat(),"reason":"downstream_replay_from_checkpoint","replay_source_run":str(source),"replay_from_stage":from_stage,"upstream_artifacts_reused":True,"raw_l1_claims_reused":sum(1 for x in (artifacts/required).read_text().splitlines() if x.strip()),"graph_observation_count":graph_count,"core_observation_count":core_count,"true_graph_conflict_count":0,"formal_hypothesis_count":0}
    ...
    version=f"v2_replay_{from_stage}";bundle,case_manifest=export_case_bundle(target,case_profile,bundle_root,bundle_id_suffix=bundle_id_suffix,overwrite_bundle=overwrite_bundle,manifest_overrides={"case_version":version,"is_replay_run":True,"is_replay":True,"replay_from_stage":from_stage,"source_run":str(source),"source_case_version":"v1_zero_claim","llm_used":False,"network_used":False,"replay_source_run":str(source)})
