...
def replay(case_profile,search_plan,source_run,from_stage,output_root,output_suffix,bundle_id_suffix,*,no_l1=True,network=False,api=False,skip_fulltext=True,skip_l7=True,overwrite_bundle=False,bundle_root="case_bundles",case_version=None):
    ...
    if from_stage=="l2":
        ...
        run_l2_abstract_step(run_dir=target,l1_mode="abstract_screening",execute=True,network=network,api=api)
        rerun.append("l2")
    ...
    if skip_fulltext:
        (artifacts/"l35_fulltext_retrieval_summary.json").write_text(json.dumps({"status":"planned_discovery_escalation","reason":"offline_replay_selected_candidates_without_acquisition","network_used":False,"candidate_paper_count":discovery["fulltext_escalation_candidate_count"],"fulltext_escalation_mode":discovery["fulltext_escalation_mode"],"fulltext_escalation_candidate_count":discovery["fulltext_escalation_candidate_count"]},indent=2)+"\n")
        for name in ("l35_fulltext_l1_summary.json","l35_fulltext_conflict_confirmation_summary.json"):(artifacts/name).write_text(json.dumps({"status":"skipped","reason":"stage_replay_no_llm_no_network","network_used":False},indent=2)+"\n")
