...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--api",action=argparse.BooleanOptionalAction,default=False);parser.add_argument("--network",action=argparse.BooleanOptionalAction,default=False);parser.add_argument("--dry-run",action="store_true");parser.add_argument("--resume",action=argparse.BooleanOptionalAction,default=True)
    parser.add_argument("--force-stage",action="append",choices=STAGES,default=[]);parser.add_argument("--stop-after",choices=STAGES);parser.add_argument("--no-atlas-sync",action="store_true");parser.add_argument("--no-publish-handoff",action="store_true");parser.add_argument("--json",action="store_true",dest="json_output")
    return parser


def _request(args) -> CaseToAtlasRequest:
    return CaseToAtlasRequest(case_id=args.case_id,case_profile_path=args.case_profile,search_plan_path=args.search_plan_file,runs_root=args.runs_root,system_b_output_root=args.system_b_output_root,database_url=args.database_url,external_data_root=args.external_data_root,network_enabled=args.network,api_enabled=args.api,resume=args.resume,force_stages=frozenset(args.force_stage),stop_after=args.stop_after,publish_handoff=not args.no_publish_handoff,atlas_sync=not args.no_atlas_sync,dry_run=args.dry_run)
...
def main(argv=None) -> int:
    ...
    if args.json_output:print(json.dumps(payload,ensure_ascii=False,sort_keys=True))
        ...
        print("stages: "+", ".join(f"{x['stage']}={x['action']}" for x in plan["stages"]));print(f"abstract_l1_api_expected: {plan['abstract_l1_api_expected']}");print(f"fulltext_l1_api_expected: {plan['fulltext_l1_api_expected']}");print(f"current_projection_case_count: {plan['current_projection_case_count']}")
    else:
