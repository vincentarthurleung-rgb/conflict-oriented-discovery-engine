...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--api",action=argparse.BooleanOptionalAction,default=False);parser.add_argument("--network",action=argparse.BooleanOptionalAction,default=False);parser.add_argument("--entity-llm-cleaner",action=argparse.BooleanOptionalAction,default=False);parser.add_argument("--offline",action="store_true");parser.add_argument("--dry-run",action="store_true");parser.add_argument("--reuse-only",action="store_true");parser.add_argument("--resume",action=argparse.BooleanOptionalAction,default=True)
...
def main(argv=None) -> int:
    ...
    if args.json_output:print(json.dumps(payload,ensure_ascii=False,sort_keys=True))
        ...
        print("stages: "+", ".join(f"{x['stage']}={x['action']}({x.get('reason')})" for x in plan["stages"]));print(f"base_run_action: {plan.get('base_run_recovery',{}).get('action')}");print(f"base_run_output: {plan.get('base_run_recovery',{}).get('output_run')}");print(f"next_stage: {plan.get('next_stage')}");print(f"expected_api_calls: {plan.get('expected_api_calls')}");print(f"expected_network_calls: {plan.get('expected_network_calls')}");print(f"abstract_l1_api_expected: {plan['abstract_l1_api_expected']}");print(f"fulltext_l1_api_expected: {plan['fulltext_l1_api_expected']}");print(f"reasoning_api_expected: {plan['reasoning_api_expected']}");print(f"reasoning_cache_hits: {plan['reasoning_cache_hits']}");print(f"context_consolidation_rebuild_expected: {plan['context_consolidation_rebuild_expected']}");print(f"current_projection_case_count: {plan['current_projection_case_count']}")
    else:
