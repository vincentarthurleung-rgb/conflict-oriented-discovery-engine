...
from code_engine.schemas.evidence import build_minimal_evidence_record
...
def build_report_blueprints(ranked_hypotheses: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for rank, hypothesis in enumerate(ranked_hypotheses, start=1):
        ...
        traces = hypothesis.get("whitebox_traceability", [])
        evidence_records = hypothesis.get("evidence_records") or [
            build_minimal_evidence_record(trace).model_dump() for trace in traces
        ]
        hyperedge_payload = hypothesis.get("hypothesis_hyperedge")
        if hyperedge_payload:
            hyperedge = HypothesisHyperedge.model_validate(hyperedge_payload)
        else:
            hyperedge = build_hypothesis_hyperedge(
                hypothesis,
                conflict_edges=hypothesis.get("conflict_edges", []),
                validation_results=[validation_result] if validation_result else [],
                coverage_verdict=hypothesis.get("coverage_verdict"),
            )
        reasoning_payload = hypothesis.get("reasoning_record") or build_reasoning_record(hyperedge).model_dump()
        probabilistic_state = hypothesis.get("probabilistic_conflict_state", {})
        if not probabilistic_state and hypothesis.get("conflict_edges"):
            probabilistic_state = hypothesis["conflict_edges"][0].get("probabilistic_state", {})
        blueprints.append(
            {
                "rank": rank,
                ...
                "whitebox_traceability": traces,
                "evidence_records": evidence_records,
                "probabilistic_conflict_state": probabilistic_state,
                "hypothesis_hyperedge": hyperedge.model_dump(),
                "reasoning_record": reasoning_payload,
                "coverage_status": hyperedge.coverage_status,
                "dry_lab_next_action": hypothesis.get("dry_lab_next_action", "manual_review_required"),
