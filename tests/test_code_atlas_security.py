...
class AtlasSecurityTests(unittest.TestCase):
    ...
    def test_role_allowed_modes_and_debug_redaction(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(reviewer.get("/api/triples?limit=1").status_code,403)
            ...
            admin_session=admin.get("/api/session").get_json();self.assertEqual(admin_session["allowed_modes"],["pharma"])
            self.assertEqual(admin.get("/api/review-items?limit=1").status_code,403);self.assertEqual(admin.get("/api/triples?limit=1").status_code,403)
            ...
            self.assertIn("developer",developer.get("/api/session").get_json()["allowed_modes"]);self.assertIn("display_priority_score_v2",developer.get("/api/triples?limit=1").get_json()["items"][0])
