...
async function login(page) {
  ...
  await expect(page.locator('#current-user')).toContainText(ownerUser);
}
...
test('owner can inspect system state without placeholder zeroes', async ({ page }) => {
  ...
  await expect(page.getByText('Schema 0007_owner_access_management')).toBeVisible();
});
...
test('pilot setup preview distinguishes cases from review items', async ({ page }) => {
  ...
  await expect(page.getByText('Review Item count is not Case count')).toBeVisible();
