...
class FulltextCacheIntegrationTests(unittest.TestCase):
    def test_selected_span_hit_and_changed_content_miss(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            seed = {"triple_id": "T", "query_hash": "Q", "seed_triple": {"subject": "s"}}
            (run / "run_state.json").write_text(json.dumps({"run_id": "R", "summary": {"triple_metadata": seed}}))
            ...
            signature = L1TaskSignature(task_family="fulltext_evidence_extraction", source_scope="span", canonical_paper_id="P", content_hash=compute_text_hash(span["text"]), schema_version="fulltext_evidence_v1", prompt_profile_id="profile", prompt_fingerprint="profile", prompt_template_hash="profile", model_provider=FailFulltextClient.__module__.split('.')[0], model_name="FailFulltextClient", model_fingerprint=hashlib.sha256(f"{FailFulltextClient.__module__}.{FailFulltextClient.__name__}".encode()).hexdigest(), domain_id="bio", domain_profile="bio", resolver_registry_hash=hashlib.sha256(b"domain_neutral_registry").hexdigest(), query_independent=False, query_hash="Q", triple_id="T", seed_triple_hash=hashlib.sha256(json.dumps(seed["seed_triple"], sort_keys=True).encode()).hexdigest(), l1_mode="progressive_fulltext")
